...
var require$$0$1 = require('fs/promises');
...
const promises_1$1 = require$$0__default["default"];
class UtilsFile {
    constructor() {
    ...
    async deleteFilePath(filePath) {
        ...
        const deleteFile = async () => {
            try {
                await promises_1$1.unlink(filePath);
            }
            catch (err) {
                unlinkRetries--;
                if (unlinkRetries > 0) {
    ...
    async waitForFilePathLock(filePath, timeoutMS = 4000) {
        ...
        return new Promise((resolve, reject) => {
            const check = async () => {
                ...
                try {
                    const stream = await promises_1$1.open(filePath, 'r+');
...
class Database {
    constructor(databaseLocation, dbName, encrypted, mode, version, isEncryption, readonly, upgDict, globalUtil) {
        ...
        this.pathDB = databaseLocation || this.fileUtil.getFilePath(dbName);
        ...
        try {
            ...
            if (!retB) {
                ...
                if (isLastModified && isSqlDeleted) {
                    ...
                    stmts += `INSERT INTO sync_table (sync_date)
                              VALUES (${date});`;
...
class CapacitorSQLite {
    ...
    async createConnection(options) {
        ...
        this.databases[connName] = new Database_1.Database(null, dbName + 'SQLite.db', encrypted, inMode, version, this.isEncryption, readonly, upgrades, this.globalUtil);
        return;
    }
    ...
    async importFromJson(options) {
        ...
        // Create the database
        const database = new Database_1.Database(null, dbName, encrypted, inMode, targetDbVersion, this.isEncryption, false, {}, this.globalUtil);
        try {
    ...
    async getNCDatabasePath(options) {
        ...
        const { database, path } = options;
        return { path: path_1.default.join(path, database) };
    }
    async createNCConnection(options) {
        console.log('createNCConnection', options);
        const { databasePath, version } = options;
        const connName = `RO_${databasePath}`;
        const conn = this.databases[connName];
        if (conn) {
            throw new Error(`Connection ${databasePath} already exists`);
        }
        try {
            const stat = await promises_1.default.stat(databasePath);
            if (!stat || !stat.isFile()) {
                throw new Error(`database ${databasePath} does not exist`);
            }
            this.databases[connName] = new Database_1.Database(path_1.default.dirname(databasePath), path_1.default.basename(databasePath), false, "no-encryption", version, this.isEncryption, true, {}, this.globalUtil);
        }
        catch (e) {
            throw new Error(`database ${databasePath} does not exist`);
        }
    ...
    async closeNCConnection(options) {
        ...
        const conn = this.databases[connName];
        if (!conn) {
            throw new Error(`Connection to ${databasePath} not available.`);
        }
        if (conn.isDBOpen()) {
            try {
                conn.dbClose();
